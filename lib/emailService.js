...
const DEFAULT_INQUIRY_NOTIFICATION_EMAILS = [
  'info@domyvitalii.cz',
  'lucie@creavita.cz'
];
...
class EmailService {
  ...
  getInquiryNotificationEmails(explicitRecipients) {
    const configuredRecipients = String(
      process.env.INQUIRY_NOTIFICATION_EMAILS ||
    ...
    const requestedRecipients = (Array.isArray(explicitRecipients)
      ? explicitRecipients
      : explicitRecipients
        ? [explicitRecipients]
        : [])
    ...
    return Array.from(new Set([
      ...DEFAULT_INQUIRY_NOTIFICATION_EMAILS,
  ...
  async sendEmail({ to, subject, text, html, replyTo }) {
    if (!this.isConfigured) {
      if (process.env.NODE_ENV === 'production') {
        const error = 'SendGrid is not configured with a valid SENDGRID_API_KEY.';
        console.error('[EMAIL SYSTEM] Configuration error:', error);
        return {
          success: false,
          provider: 'sendgrid',
          error
  ...
  async sendInternalInquiryNotification({
    ...
    const notificationEmails = this.getInquiryNotificationEmails(
